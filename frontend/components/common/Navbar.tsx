...
export default function Navbar({ onHoverMenuChange, hoveredMenu }: NavbarProps) {
  ...
  const activeMenuGrid = hoveredMenu === "services" ? servicesItems : locationsItems;

  const megaMenuLayoutVariants = {
    hidden: {
  ...
  return (
    <>
      <nav className="absolute top-0 left-0 w-full z-50 px-6 md:px-11 py-6 md:py-8 flex items-center justify-between">
        {/* Brand / Logo */}
        <Link
          href="/"
        ...
        <div className={`hidden xl:flex items-center bg-[#F0ECE7]/15 backdrop-blur-[24px] rounded-full p-1.5 gap-3 relative transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${isMobileMenuOpen ? 'opacity-0 -translate-y-8 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
          <Link
            href="/"
            onMouseEnter={() => onHoverMenuChange?.(null)}
            className="px-5 py-3 text-base font-medium text-white bg-[#FFBB78]/12 backdrop-blur-[24px] hover:bg-[#FFBB78]/20 rounded-full transition-colors tracking-normal font-[Geist] leading-none"
          >
            HOME
          </Link>
          <Link
          ...
          >
            <div className={`pl-6 pr-1.5 py-1.5 text-base font-medium text-white bg-[#FFBB78]/12 backdrop-blur-[24px] hover:bg-[#FFBB78]/20 rounded-full transition-all duration-300 flex items-center gap-5 ${hoveredMenu === "services" ? "bg-[#FFBB78]/20" : ""}`}>
              <Link href="/service" className="tracking-normal font-[Geist] leading-none">SERVICES</Link>
          ...
          <AnimatePresence>
            {hoveredMenu && (
              <motion.div
                ...
                className={`absolute left-1/2 -translate-x-1/2 top-16 bg-[#DED7CE] border border-white/40 shadow-2xl rounded-[1.5rem]  z-50 ${hoveredMenu === "services" ? "w-[650px] flex flex-col" : "w-[560px] grid grid-cols-2 gap-3.5"
                  }`}
              >
                {hoveredMenu === "services" ? (
                  isLoadingServices ? (
                    ...
                    Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="p-4 flex items-center gap-8 border-b border-black/5 last:border-0 animate-pulse">
                        <div className="w-[180px] h-[115px] rounded-xl bg-black/5 shrink-0" />
                        <div className="flex-1 space-y-2">
                          <div className="h-5 bg-black/5 rounded w-1/2" />
                        </div>
                      </div>
                    ))
                      ...
                      return (
                        ...
                        >
                          <Link
                            ...
                            className="p-4 border-b border-black/5 last:border-0 flex items-center justify-between cursor-pointer transition-all duration-300 hover:bg-black/[0.03] group rounded-xl"
                          >
                            <div className="flex items-center gap-8">
                              {/* Left Image */}
                              <div className="relative w-[180px] h-[115px] rounded-xl overflow-hidden shrink-0 shadow-sm">
                                <img
                              ...
                              {/* Center Content */}
                              <div className="flex flex-col flex-1 text-left justify-center pr-4">
                                <span className="font-sans text-[1.2rem] md:text-[1.4rem] font-medium text-black/80 group-hover:text-black transition-colors">
                                  {item.title}
                                </span>
                                {item.description && (
                                  <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] opacity-0 group-hover:opacity-100 transition-all duration-300 ease-out">
                                    <div className="overflow-hidden min-h-0">
                                      <p className="font-sans text-[0.95rem] leading-[1.4] text-black/60 line-clamp-2 mt-1">
                                        {item.description}
                                      </p>
                                    </div>
                                  </div>
                            ...
                            {/* Right Plus */}
                            <div className="text-black/60 group-hover:text-black transition-colors shrink-0 mr-4">
                              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="M12 5v14" /></svg>
                            </div>
                          </Link>
                        </motion.div>
                      );
        ...
        <button
          ...
          onMouseEnter={() => onHoverMenuChange?.(null)}
          className="flex items-center gap-2 bg-[#EFE9E1]/90 backdrop-blur-sm text-black hover:bg-white px-4 md:px-5 py-3 md:py-4 rounded-xl transition-all duration-300 text-sm md:text-base shadow-sm hover:shadow-md active:scale-95"
        >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              variants={megaMenuLayoutVariants}
              initial="hidden"
              animate="visible"
              exit="hidden"
            ...
            >
              ...
              <div className="flex-1 flex w-full">
                {/* Left side SVG Icon */}
                <motion.div variants={megaMenuChildVariants} className="flex-1 flex items-start p-16">
                ...
                <div className="flex gap-34 justify-between py-8 pl-0 ">
                  ...
                  <div className="flex flex-col gap-12 font-[Geist] pt-4">
                    <motion.div variants={megaMenuChildVariants}>
                  ...
                  {/* Primary Main Links */}
                  <div className="flex flex-col gap-2 font-[Geist] pr-12">
                    {["Home", "About", "Conditions", "Treatments", "Cases", "Equipment", "Gallery", "Blog", "Contact", "Locations"].map((link) => (
                      <motion.div key={link} variants={megaMenuChildVariants}>
                        <Link
              ...
              {/* <motion.div variants={megaMenuChildVariants} className="px-16 pb-12 flex items-end justify-between mt-auto">
                <Link href="#" className="text-[11px] font-semibold uppercase tracking-[0.1em] text-black border-b border-black/40 hover:border-black pb-1.5 w-max transition-colors font-mono">
                  MADE BY METRIK.STUDIO
                </Link>
                <div className="flex gap-8 items-center">
                  <Link href="#" className="text-[11px] font-semibold uppercase tracking-[0.1em] text-black/70 hover:text-black border-b border-black/20 hover:border-black/40 pb-1.5 w-max transition-colors font-mono">
                    MORE WEBFLOW TEMPLATES
                  </Link>
                  <Link href="#" className="text-[11px] font-semibold uppercase tracking-[0.1em] text-black hover:text-black border-b border-black/40 hover:border-black pb-1.5 w-max flex items-center gap-2 transition-colors font-mono group">
                    BUY A TEMPLATE
                    <svg className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z" /><path d="M3 6h18" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>
                  </Link>
                </div>
              </motion.div> */}
            </motion.div>
            ...
            >
              ...
              {/* Main Links */}
              <div className="flex flex-col gap-2 mt-4 font-[Geist]">
                {["Home", "About", "Services", "Cases", "Team", "Blog", "Contact", "Locations"].map((link) => {
                  if (link === "Services") {
                    return (
              ...
              {/* Secondary Sections */}
              <div className="mt-14 flex flex-col gap-8 font-[Geist]">
                <div>
              ...
              {/* <div className="mt-auto pt-16 flex flex-col gap-4">
                <Link href="#" className="text-[10px] font-bold uppercase tracking-[0.1em] text-black/70 hover:text-black border-b border-black/20 pb-1.5 w-max transition-colors font-mono">
                  MADE BY METRIK.STUDIO
                </Link>
                <Link href="#" className="text-[10px] font-bold uppercase tracking-[0.1em] text-black/70 hover:text-black border-b border-black/20 pb-1.5 w-max transition-colors font-mono">
                  MORE WEBFLOW TEMPLATES
                </Link>
                <Link href="#" className="text-[10px] font-bold uppercase tracking-[0.1em] text-black/70 hover:text-black border-b border-black/20 pb-1.5 w-max flex items-center gap-2 transition-colors font-mono">
                  BUY A TEMPLATE
